...
public class NewTagServlet extends HttpServlet {
	...
	public NewTagServlet() {
		super();
	}

	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		getServletContext().getRequestDispatcher("/WEB-INF/jsp/newTag.jsp").forward(request, response);
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (errors.isEmpty()) {
			...
			if (sessionUserName != null) {
				...
				if (user != null) {
					tagService.addNewTag(name, user.getId());
				}
			}
	...
	private Map<String, String> getErrors(String name) {
		...
		} else if (tagService.nameExists(name)) {
